    ...
    public partial class Candidateapplication : Clcookiehandler
    {
        ...
        public void adddata(string __fname)
        {
            ...
            if (temphldoldcan.Length > 0)
            {
                ...
                app.AddApplicationcan(mxappid, temphldoldcan[4], temphldoldcan[0], temphldoldcan[1],
                                      dobdate1,
                                      TextBox2.Text, mxdocid, Session["pusername"].ToString());
                app.AddApplicationMapping(__guid.GenerateGuid(), Request.QueryString["JobID"], mxappid);
                app.Adddocuments(mxdocid, __pcount + __fname, pathsetter + __pcount + __fname);
            }
        }
                ...
                if (__ext != "none")
                {
                    if (FileUpload1.FileContent.Length < __maxuploadsz)
                    {
                        ...
                        try
                        {
                            ...
                            emp.Sendappemaildbupdate(guids.GenerateGuid(),  Session["pusername"].ToString(), 1);
                        }
                        catch (Exception)
                        {
                            Console.Write("should not reach here!");
                        }
