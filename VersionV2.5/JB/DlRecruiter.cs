...
namespace JB
{
    public class DlRecruiter
    {
        ...
        public int Getrecwjobscount()
        {
            ...
            using (connreader)
            {
                var command = new MySqlCommand(@"select count(*) FROM getallrec;",connreader);
                connreader.Open();
        ...
        public DataSet Getallrecwithjobs(int minid, int maxid)
        {
            var ds = new DataSet();
            ...
            var myda = new MySqlDataAdapter(@"select * from getallrec limit "+minid+","+maxid+" ; ", mycon);
