...
namespace JB
{
    public class Clpager: Page
    {
        public string CreatePageLinks(int totalRecords, int pageSize)
        {
            ...
            if (currentPage > 1)
                strPager.Append(string.Format("<a class=\"ux_page\" title=\"Previous page\" href=\"{1}?page={0}\">Prev</a>", (currentPage - 2) + 1, pageUrl));
            int min, max;
            ...
            for (int n = min; n <= max; n++)
            {
                if (n > totalPages)
                    break;
                if (n == currentPage)
                    strPager.Append(String.Format("<div class=\"ux_pagecurrent\">{0}</div>", n.ToString(CultureInfo.InvariantCulture)));
                else
                    strPager.Append(String.Format("<a class=\"ux_page\" title=\"{0}\" href=\"{2}?page={1}\">{0}</a> ", n, (n - 1) + 1, pageUrl));
            }
            if (currentPage < totalPages)
                strPager.Append(String.Format("<a class=\"ux_page\" title=\"{0}\" href=\"{1}?page={0}\">Next</a>", currentPage + 1, pageUrl));
